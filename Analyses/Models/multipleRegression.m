function [beta,pred,mse,Filter,combinations] = multipleRegression(Stim, Data, numLags, varargin)
...
beta = [ones(T,1),Stim]\Data; % compute regression with DC coefficient
% m = mean(Data(:));                % compute mean
% beta = [ones(T,1),Stim]\(Data-m); % compute regression with DC coefficient on zeroed data
...
% pred = [ones(T,1),Stim']*beta + m; % compute prediction and add back mean
