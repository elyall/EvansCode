function [Id,dpx,dpy] = doLucasKanade_gpu(T,I,dpx,dpy,Parameters)
warning('off','fastBSpline:nomex');
if nargin < 5
    Nbasis = 16;
    niter = 25;
    damping = 1;
    deltacorr = gpuArray(.0005);
else
    Nbasis = Parameters.Nbasis;
    niter = Parameters.niter;
    damping = Parameters.damping;
    deltacorr = gpuArray(Parameters.deltacorr);
end
...
if nargin < 3 || isempty(dpx) || isempty(dpy)
...
B = gpuArray(spl.getBasis((1:size(T,1))'));
...
for ii = 1:niter
    ...
    Hx = constructH(gather(allBs'*sum(dTx.^2,2)),size(B,2))+theI;
    Hy = constructH(gather(allBs'*sum(dTy.^2,2)),size(B,2))+theI;
